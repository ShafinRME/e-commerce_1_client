            ...
            {users.length > 0 && (
                <div className="overflow-x-auto shadow-md  bg-white">
                    <table className="table  w-full">
                        ...
                        <tbody>
                            {users.map((u, idx) => (
                                <tr key={u._id} className={idx % 2 === 0 ? "bg-slate-200" : "bg-slate-300"}>
                                    <td>{u.email}</td>
                                    <td>{new Date(u.created_at).toLocaleDateString()}</td>
                                    <td>
                                        <span
                                            className={`badge ${u.role === "admin" ? "badge-success" : "badge-accent"
                                    ...
                                    <td>
                                        <button
                                            ...
                                            className={`btn btn-sm  text-black ${u.role === "admin" ? "btn-error" : "btn-secondary"
                                                }`}
                                        >
                                            {u.role === "admin" ? (
                                                <>
